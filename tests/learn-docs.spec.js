...
test('Learn dark documentation retains its native primary and secondary surfaces', async ({ page }) => {
  ...
  await expect(page.getByTestId('selected-nav')).toHaveCSS('background-color', 'rgb(48, 48, 48)')
  await expect(page.getByTestId('selected-nav')).toHaveCSS('color', 'rgb(184, 184, 192)')
  ...
  expect(grays).toEqual({
    50: '#131313', 100: '#181818', 200: '#212121', 300: '#303030',
    400: '#414141', 500: '#5d5d5d', 600: '#8f8f8f', 700: '#afafaf',
    750: '#b9b9b9', 800: '#cdcdcd', 900: '#ededed', 950: '#f3f3f3'
  })
})
...
test('the shared ChatGPT body alone reproduces Learn surface washout', async ({ page }) => {
  ...
  await expect(page.getByTestId('selected-nav')).toHaveCSS('background-color', 'rgb(184, 184, 192)')
})
